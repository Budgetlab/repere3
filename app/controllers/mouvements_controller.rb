class MouvementsController < ApplicationController
  ...
  def index
    ...
    when 'admin'
      @redeploiements = Redeploiement.includes(:region, mouvements: [:service, :programme]).where(created_at: @start..).order(created_at: :desc)
      ...
      @etp_cible = Objectif.since_date(@start).sum(:etp_cible)
    when 'prefet', 'CBR'
      @redeploiements = Redeploiement.includes(mouvements: [:service, :programme]).where(region_id: current_user.region_id, created_at: @start..).order(created_at: :desc)
      @mouvements_array = @redeploiements.pluck(:redeploiement_id, :type_mouvement, 'mouvements.quotite', 'mouvements.cout_etp', 'mouvements.credits_gestion')
      @etp_cible = Objectif.since_date(@start).where(region_id: current_user.region_id).sum(:etp_cible)
    when 'ministere'
      @ministere = Ministere.where(nom: current_user.nom).first
      @programme_id = Programme.where(ministere_id: @ministere.id).pluck(:id)
      @mouvements = Mouvement.includes(:service, :programme).since_date(@start).where(programme_id: @programme_id).order(created_at: :desc)
      @etp_cible = Objectif.since_date(@start).where(programme_id: @programme_id).sum(:etp_cible)
      @redeploiements = []
